...
static int subboard_verify(struct devinfo_data *const devinfo_data)
{
	...
	switch(get_project()) {
		...
		case OPPO_19747:
			...
			break;
		case OPPO_206AC: {
//Haibo.Dong@ODM_WT.Kernel.Boardid, 2020/10/14, add pascal-I sub board match
			if ((id1 == 0) &&
				(operator == OPERATOR_206A_ASIA_SIMPLE ||
				operator == OPERATOR_206C_ASIA_SIMPLE ||
				operator == OPERATOR_206I_ASIA_SIMPLE ||
				operator == OPERATOR_206I_ASIA_SIMPLE_3C)) {
				snprintf(devinfo_data->sub_mainboard_info.manufacture, INFO_BUF_LEN, "-match");
			 } else if ((id1 == 1) &&
				(operator == OPERATOR_206A_All_BAND ||
				operator == OPERATOR_206C_All_BAND ||
				operator == OPERATOR_206I_All_BAND ||
				operator == OPERATOR_206I_All_BAND_3C)) {
				snprintf(devinfo_data->sub_mainboard_info.manufacture, INFO_BUF_LEN, "-match");
			 }else {
				snprintf(devinfo_data->sub_mainboard_info.manufacture, INFO_BUF_LEN, "-unmatch");
			}
...
//Jiantao.Liu@ODM_WT.MM.Display.Lcd, 2020/07/27, switch of LCD backlight current 24.5MA
			if ((0x02 == get_PCB_Version()) || (0x03 == get_PCB_Version()))
			backlight_current_switch = 1;
//Jiantao.Liu@ODM_WT.MM.Display.Lcd, 2020/07/30, Modify  DRE&CABC for backlight hight light mode
			backlight_high_light_dre_cabc = 1;
			}
			if((operator == OPERATOR_206I_ASIA_SIMPLE) || (operator == OPERATOR_206I_All_BAND)){
				backlight_current_switch = 1;
//Hao.Liang@ODM_WT.MM.Display.Lcd, 2020/12/25, pascal IModify  DRE&CABC for backlight hight light mode
				backlight_high_light_dre_cabc = 1;
			}
			break;
		default:
...
static void init_proc_devinfo_modify(void){
	struct proc_dir_entry *p = NULL;
#ifndef ODM_WT_EDIT
/*Haibo.dong@ODM_WT.BSP.Kernel.Boot, 2020/07/13, Add for set devinfo_modify in rc*/
	p = proc_create("devinfo_modify", S_IWUSR | S_IWGRP | S_IWOTH, NULL, &devinfo_modify_fops);
#else
	p = proc_create("devinfo_modify", S_IWUSR | S_IWGRP | S_IWOTH | S_IRUSR | S_IRGRP | S_IROTH, NULL, &devinfo_modify_fops);
#endif  /**/
	if(!p)
