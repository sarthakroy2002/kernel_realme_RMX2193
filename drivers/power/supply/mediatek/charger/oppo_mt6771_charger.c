...
{
	struct oppo_chg_chip *chg_chip =
	container_of(alarm, struct oppo_chg_chip, usbotp_recover_timer);
	...
	if(chg_chip->chg_ops->oppo_chg_set_hz_mode != NULL)
		chg_chip->chg_ops->oppo_chg_set_hz_mode(false);

	wake_up_process(oppo_usbtemp_kthread);
...
#ifdef ODM_WT_EDIT
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/8/4, add for usb port over temp protection*/
	alarm_init(&g_oppo_chip->usbotp_recover_timer, ALARM_BOOTTIME, recover_charge_hrtimer_func);
#endif /*ODM_WT_EDIT*/
