...
/* Junbo.Guo@ODM_WT.BSP.CHG.Basic, 20191109,Add for charger connect error */
	...
	rt9471_i2c_update_bits(chip, RT9471_REG_FUNCTION,
				      0 << RT9471_BATFETDIS_DELAY_SHIFT,
				      RT9471_BATFETDIS_DELAY_MASK);
	ret = __rt9471_enable_shipmode(chip, true);
	if (ret < 0) {
		dev_notice(chip->dev, "%s enter shutdown mode fail(%d)\n",
				__func__, ret);
	}
	ret = rt9471_i2c_read_byte(chip, RT9471_REG_FUNCTION, &regval);
	if (ret < 0){
		dev_notice(chip->dev, "%s read 0x02 fail(%d)\n",
				__func__, ret);
	}else{
		dev_info(chip->dev, "%s reg0x%02X = 0x%02X\n", __func__,
					    RT9471_REG_FUNCTION, regval);
	}
}
