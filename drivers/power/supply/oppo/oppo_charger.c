...
extern int get_flashlight_temperature(void);
...
#ifdef ODM_WT_EDIT
/*Sidong.Zhao@ODM_WT.BSP.CHG 2020/8/7, battery status is charging while battery is not full*/
...
#define BATT_TEMP_EXTEND_T0_I 160
#define BATT_TEMP_EXTEND_T0_MINUS_X_I 140
#define BATT_TEMP_EXTEND_T1_I 220
#define BATT_TEMP_EXTEND_T1_MINUS_X_I 200
#define BATT_TEMP_EXTEND_T2_I 300
#define BATT_TEMP_EXTEND_T2_MINUS_X_I 280
#define BATT_TEMP_EXTEND_T3_I 360
#define BATT_TEMP_EXTEND_T3_MINUS_X_I 340
#define BATT_TEMP_EXTEND_T4_I 380
#define BATT_TEMP_EXTEND_T4_MINUS_X_I 360
#define BATT_TEMP_EXTEND_T5_I 400
#define BATT_TEMP_EXTEND_T5_MINUS_X_I 380
#define BATT_TEMP_EXTEND_T6_I 420
#define BATT_TEMP_EXTEND_T6_MINUS_X_I 400
#define BATT_TEMP_EXTEND_T7_I 450
#define BATT_TEMP_EXTEND_T7_MINUS_X_I 430


#define BATT_TEMP_EXTEND_T0_TO_T1_ICHG_I 3000
#define BATT_TEMP_EXTEND_T1_TO_T2_ICHG_PART1_I 3400 //VBAT<=4.18 
#define BATT_TEMP_EXTEND_T1_TO_T2_ICHG_PART2_I 3000  //VBAT>4.18
#define BATT_TEMP_EXTEND_T2_TO_T3_ICHG_I 3300
#define BATT_TEMP_EXTEND_T3_TO_T4_ICHG_I 2400
#define BATT_TEMP_EXTEND_T4_TO_T5_ICHG_I 1700 //need vbus to 5v
#define BATT_TEMP_EXTEND_T4_TO_T5_ICHG2_I 1700
#define BATT_TEMP_EXTEND_T4_TO_T5_ICHG3_I 1700
#define BATT_TEMP_EXTEND_T5_TO_T6_ICHG_I 1600 //need vbus to 5v
#define BATT_TEMP_EXTEND_T6_TO_T7_ICHG_I 1600 //need vbus to 5v
...
static void oppo_chg_check_thermal_on_ichging(struct oppo_chg_chip *chip)
{
	...
	if(chip->is_double_charger_support){
		if (batt_temp >= chip->limits.batt_temp_extend_t7) {
			chip->jeita_sm = BATT_TEMP_EXTEND_ABOVE_T7;
		} else if (batt_temp >= chip->limits.batt_temp_extend_t6) {
			if ((BATT_TEMP_EXTEND_ABOVE_T7 != chip->jeita_sm )
				|| (batt_temp <= chip->limits.batt_temp_extend_t7_minus_x)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T6_TO_T7;
			}
		} else if (batt_temp >= chip->limits.batt_temp_extend_t5) {
			if ((BATT_TEMP_EXTEND_T6_TO_T7 != chip->jeita_sm )
				|| (batt_temp <= chip->limits.batt_temp_extend_t6_minus_x)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T5_TO_T6;
			}
		}else if (batt_temp >= chip->limits.batt_temp_extend_t4) {
			if ((BATT_TEMP_EXTEND_T5_TO_T6 != chip->jeita_sm )
				|| (batt_temp <= chip->limits.batt_temp_extend_t5_minus_x)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T4_TO_T5;
			}
		}else if (batt_temp >= chip->limits.batt_temp_extend_t3) {
			if ((BATT_TEMP_EXTEND_T4_TO_T5 != chip->jeita_sm )
				|| (batt_temp <= chip->limits.batt_temp_extend_t4_minus_x)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T3_TO_T4;
			}
		} else if (batt_temp >= chip->limits.batt_temp_extend_t2) {
			if ((BATT_TEMP_EXTEND_T3_TO_T4 != chip->jeita_sm )
				|| (batt_temp <= chip->limits.batt_temp_extend_t3_minus_x)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T2_TO_T3;
			}
		} else if (batt_temp >= chip->limits.batt_temp_extend_t1) {
			if ((BATT_TEMP_EXTEND_T2_TO_T3 != chip->jeita_sm )
				|| (batt_temp <= chip->limits.batt_temp_extend_t2_minus_x)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T1_TO_T2;
			}
		} else if (batt_temp >= chip->limits.batt_temp_extend_t0) {
			if ((BATT_TEMP_EXTEND_T1_TO_T2 != chip->jeita_sm )
				|| (batt_temp <= chip->limits.batt_temp_extend_t1_minus_x)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T0_TO_T1;
			}
		}else {
			if ((BATT_TEMP_EXTEND_T0_TO_T1 != chip->jeita_sm)
				|| (batt_temp <= chip->limits.batt_temp_extend_t0_minus_x)) {
...
static void oppo_chg_voter_charging_stop(struct oppo_chg_chip *chip, OPPO_CHG_STOP_VOTER voter)
{
        ...
        case CHG_STOP_VOTER__VCHG_ABNORMAL:
                ...
                break;
#ifdef ODM_WT_EDIT
//Sidong.Zhao@WT.CHG,2020/7/10,modify for charge protection
		case CHG_STOP_VOTER__BATTTEMP_ABNORMAL:
			chip->charging_state = CHARGING_STATUS_FAIL;
			chip->total_time = 0;
			oppo_chg_turn_off_charging(chip);
...
/*Sidong.Zhao@ODM_WT.BSP.CHG 2019/11/27,AP thermal machine*/
	...
	if((get_Operator_Version() == OPERATOR_206I_ASIA_SIMPLE) ||   /*for pascal i*/
		(get_Operator_Version() == OPERATOR_206I_All_BAND)){
		pr_info("use pascal i charge policy\n");
		chip->limits.batt_temp_extend_t0 = BATT_TEMP_EXTEND_T0_I;
		chip->limits.batt_temp_extend_t0_minus_x = BATT_TEMP_EXTEND_T0_MINUS_X_I;
		chip->limits.batt_temp_extend_t1 = BATT_TEMP_EXTEND_T1_I;
		chip->limits.batt_temp_extend_t1_minus_x = BATT_TEMP_EXTEND_T1_MINUS_X_I;
		chip->limits.batt_temp_extend_t2 = BATT_TEMP_EXTEND_T2_I;
		chip->limits.batt_temp_extend_t2_minus_x = BATT_TEMP_EXTEND_T2_MINUS_X_I;
		chip->limits.batt_temp_extend_t3 = BATT_TEMP_EXTEND_T3_I;
		chip->limits.batt_temp_extend_t3_minus_x = BATT_TEMP_EXTEND_T3_MINUS_X_I;
		chip->limits.batt_temp_extend_t4 = BATT_TEMP_EXTEND_T4_I;
		chip->limits.batt_temp_extend_t4_minus_x = BATT_TEMP_EXTEND_T4_MINUS_X_I;
		chip->limits.batt_temp_extend_t5 = BATT_TEMP_EXTEND_T5_I;
		chip->limits.batt_temp_extend_t5_minus_x = BATT_TEMP_EXTEND_T5_MINUS_X_I;
		chip->limits.batt_temp_extend_t6 = BATT_TEMP_EXTEND_T6_I;
		chip->limits.batt_temp_extend_t6_minus_x = BATT_TEMP_EXTEND_T6_MINUS_X_I;
		chip->limits.batt_temp_extend_t7 = BATT_TEMP_EXTEND_T7_I;
		chip->limits.batt_temp_extend_t7_minus_x = BATT_TEMP_EXTEND_T7_MINUS_X_I;

		chip->limits.batt_temp_extend_t0_to_t1_ichg = BATT_TEMP_EXTEND_T0_TO_T1_ICHG_I;
		chip->limits.batt_temp_extend_t1_to_t2_ichg_part1 = BATT_TEMP_EXTEND_T1_TO_T2_ICHG_PART1_I;
		chip->limits.batt_temp_extend_t1_to_t2_ichg_part2 = BATT_TEMP_EXTEND_T1_TO_T2_ICHG_PART2_I;
		chip->limits.batt_temp_extend_t2_to_t3_ichg = BATT_TEMP_EXTEND_T2_TO_T3_ICHG_I;
		chip->limits.batt_temp_extend_t3_to_t4_ichg = BATT_TEMP_EXTEND_T3_TO_T4_ICHG_I;
		chip->limits.batt_temp_extend_t4_to_t5_ichg = BATT_TEMP_EXTEND_T4_TO_T5_ICHG_I;
		chip->limits.batt_temp_extend_t4_to_t5_ichg2 = BATT_TEMP_EXTEND_T4_TO_T5_ICHG2_I;
		chip->limits.batt_temp_extend_t4_to_t5_ichg3 = BATT_TEMP_EXTEND_T4_TO_T5_ICHG3_I;
		chip->limits.batt_temp_extend_t5_to_t6_ichg = BATT_TEMP_EXTEND_T5_TO_T6_ICHG_I;
		chip->limits.batt_temp_extend_t6_to_t7_ichg = BATT_TEMP_EXTEND_T6_TO_T7_ICHG_I;
	} else {	/*for pascal a*/
		pr_info("use pascal a charge policy\n");
		chip->limits.batt_temp_extend_t0 = BATT_TEMP_EXTEND_T0;
		chip->limits.batt_temp_extend_t0_minus_x = BATT_TEMP_EXTEND_T0_MINUS_X;
		chip->limits.batt_temp_extend_t1 = BATT_TEMP_EXTEND_T1;
		chip->limits.batt_temp_extend_t1_minus_x = BATT_TEMP_EXTEND_T1_MINUS_X;
		chip->limits.batt_temp_extend_t2 = BATT_TEMP_EXTEND_T2;
		chip->limits.batt_temp_extend_t2_minus_x = BATT_TEMP_EXTEND_T2_MINUS_X;
		chip->limits.batt_temp_extend_t3 = BATT_TEMP_EXTEND_T3;
		chip->limits.batt_temp_extend_t3_minus_x = BATT_TEMP_EXTEND_T3_MINUS_X;
		chip->limits.batt_temp_extend_t4 = BATT_TEMP_EXTEND_T4;
		chip->limits.batt_temp_extend_t4_minus_x = BATT_TEMP_EXTEND_T4_MINUS_X;
		chip->limits.batt_temp_extend_t5 = BATT_TEMP_EXTEND_T5;
		chip->limits.batt_temp_extend_t5_minus_x = BATT_TEMP_EXTEND_T5_MINUS_X;
		chip->limits.batt_temp_extend_t6 = BATT_TEMP_EXTEND_T6;
		chip->limits.batt_temp_extend_t6_minus_x = BATT_TEMP_EXTEND_T6_MINUS_X;
		chip->limits.batt_temp_extend_t7 = BATT_TEMP_EXTEND_T7;
		chip->limits.batt_temp_extend_t7_minus_x = BATT_TEMP_EXTEND_T7_MINUS_X;
...
/* Tongxing.Liu@ODM_WT.BSP.Tp.Funtion, 20191111,Add for tp recognoise usb plug in or out */
			...
			else 
			{
				...
				/*Shouli.Wang@ODM_WT.BSP.CHG 2020/12/14, add for update charger type*/
...
static void battery_notify_flag_check(struct oppo_chg_chip *chip)
{
        if (chip->notify_code & (1 << NOTIFY_CHGING_OVERTIME)) {
                ...
                chip->notify_flag = NOTIFY_BAT_FULL;
        } else if(chip->notify_code & (1 << NOTIFY_FLASHLIGHT_TERMINAL) {
				chip->notify_flag = NOTIFY_FLASHLIGHT_TERMINAL;
		} else {
                chip->notify_flag = 0;
        }
}
...
//Dong.Qi.WT.BSP.POWER.2021.01.22,S98533NOTIFY_FLASHLIGHT_TERMINAL
static void flashlight_notify_flag_check(struct oppo_chg_chip *chip)
{
	static int count = 0;

	if (pascal_project() == 5) {

		if(get_flashlight_temperature() >= 49000) {
			count++;
			if(count >= 2) {
				printk("notify flash_ntc tmep %d\n", get_flashlight_temperature());
				chip->notify_code |= 1 << NOTIFY_FLASHLIGHT_TERMINAL;
				count = 2;
			}
		} else {
			count = 0;
...
static void oppo_chg_battery_notify_check(struct oppo_chg_chip *chip)
{
		...
		flashlight_notify_flag_check(chip);
}
...
#ifdef ODM_WT_EDIT
/*Sidong.Zhao@WT.CHG, 2020/8/18, if vbat is almost full, need to disable sub chargingIC,let main chargeIC continue charging until ichg lower than iterm*/
...
static void oppo_chg_pdqc_to_normal(struct oppo_chg_chip *chip)
{
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PD) {
		if ((chip->batt_volt > chip->limits.vbatt_pdqc_to_5v_thr && pdqc_9v == true&&chip->ui_soc>=85&&chip->icharging > -1000)
			|| (chip->in_rechging && chip->charger_volt > 7500)) {
			ret = chip->chg_ops->oppo_chg_pd_setup();
			if (ret >= 0) {
				pdqc_9v = false;
				chip->limits.temp_normal_fastchg_current_ma = chip->limits.default_temp_normal_fastchg_current_ma;
