...
*/
			...
			data->pins_spi_cs_mode = pinctrl_lookup_state(data->pinctrl_gpios, "spi_cs_mode");
						if (IS_ERR(data->pins_spi_cs_mode)) {
							ret = PTR_ERR(data->pins_spi_cs_mode);
							data->pins_spi_cs_mode = NULL;
							printk(KERN_ERR "%s can't find fingerprint pinctrl spi_cs_mode\n", __func__);
							//goto pinctrl_err;
						}
