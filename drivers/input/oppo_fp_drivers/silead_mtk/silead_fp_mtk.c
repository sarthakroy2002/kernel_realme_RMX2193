...
static int silfp_parse_dts(struct silfp_data* fp_dev)
{
    ...
    fp_dev->pin.pins_spi_cs_mode= pinctrl_lookup_state(fp_dev->pin.pinctrl, "spi_cs_mode");
    if (IS_ERR_OR_NULL(fp_dev->pin.pins_spi_cs_mode)) {
        fp_dev->pin.pins_spi_cs_mode = NULL;
        LOG_MSG_DEBUG(ERR_LOG, "%s can't find silfp pins_spi_cs_mode\n", __func__);
        // Ignore error
    }
#endif /* BSP_SIL_POWER_SUPPLY_PINCTRL */
...
static int silfp_resource_init(struct silfp_data *fp_dev, struct fp_dev_init_t *dev_info)
{
    ...
    silfp_hw_poweron(fp_dev);
    if(fp_dev->pin.pins_spi_cs_mode){
        pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.pins_spi_cs_mode);
    }
    pinctrl_select_state(fp_dev->pin.pinctrl, fp_dev->pin.pins_rst_h);
