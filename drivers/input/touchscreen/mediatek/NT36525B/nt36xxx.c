...
#define FTS_UPGRADE_HLT_BOE_B8_FILE "firmware_nvt/hlt_boe_b8_fw.h"
...
const u8 up_fw_kernel_hlt_boe_b8[] = {
#include FTS_UPGRADE_HLT_BOE_B8_FILE
};
...
struct upgrade_fw_info nvt_fw_list[] = {
    ...
    {HLT_BOE_B8, "HLT_B8", BOOT_UPDATE_FIRMWARE_HLT_BOE_B8_NAME,  MP_UPDATE_FIRMWARE_HLT_BOE_B8_NAME, up_fw_kernel_hlt_boe_b8, sizeof(up_fw_kernel_hlt_boe_b8), OPPO_BOOT_UPDATE_FIRMWARE_HLT_BOE_B8_NAME},
};
...
info_retry:
	...
	} else if (ts->vendor_id == 3) {
		sprintf(Ctp_name,"HLT_B8,NT525B,FW:0x%x\n",ts->fw_ver);
		sprintf(version,"HLT-B8_NT525B_0x%02x",ts->fw_ver);
		devinfo_info_tp_set(version, "HLT",OPPO_BOOT_UPDATE_FIRMWARE_HLT_BOE_B8_NAME);
	} else {
		sprintf(Ctp_name,"HLT,NT525B,FW:0x%x\n",ts->fw_ver);
        sprintf(version,"HLT_NT525B_0x%02x",ts->fw_ver);
...
//Bin.Su@ODM_WT.BSP.Tp.Init.1644722,2018/11/21,Bringup nt36525b+huaxian tp
	...
	if ( nt_ctpmodule == 0 ) {
		...
		if (nt_ctpmodule == 0) {
			...
			nt_ctpmodule = strncmp(temp,"nt36525b_hlt_psc_ac_vdo",strlen("nt36525b_hlt_psc_ac_vdo"));
			if (nt_ctpmodule == 0) {
				NVT_LOG("this is hlt_boe_b8 nt36525b touchscreen\n");
				nt_ctpmodule = 3;
			} else {
