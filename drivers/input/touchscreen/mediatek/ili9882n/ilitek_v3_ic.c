...
out:
	...
	if (ili_tp_module == MODEL_LS) {
		...
		devinfo_info_tp_set(ili9882_version, "LS", LS_FW_SIGN_FILP_PATH);
	} else if (ili_tp_module == MODEL_TRULY) {
		sprintf(Ctp_name,"TRULY,ILI9882,FW:0x%x\n",buf[3]);
		sprintf(ili9882_version,"TRULY_Ili9882_%0x",buf[3]);
		devinfo_info_tp_set(ili9882_version, "TRULY", TRULY_FW_SIGN_FILP_PATH);
	} else if (ili_tp_module == MODEL_TXD) {
		sprintf(Ctp_name,"TXD,ILI9882,FW:0x%x\n",buf[3]);
		sprintf(ili9882_version,"TXD_Ili9882_%0x",buf[3]);
		devinfo_info_tp_set(ili9882_version, "TXD", TXD_FW_SIGN_FILP_PATH);
	} else {
		sprintf(Ctp_name,"DEFAULT,ILI9882,FW:0x%x\n",buf[3]);
		sprintf(ili9882_version,"DEFAULT_Ili9882_%0x",buf[3]);
		devinfo_info_tp_set(ili9882_version, "DEFAULT", LS_FW_SIGN_FILP_PATH);
	}
