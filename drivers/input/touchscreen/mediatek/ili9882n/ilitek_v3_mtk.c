...
void ili_input_register(void)
{
	...
	input_set_capability(ilits->input, EV_KEY, KEY_F4);/*ritchie add for oppo*/

	__set_bit(KEY_F4, ilits->input->keybit);/*ritchie add for oppo*/
...
static int __parse_tag_videolfb(struct device_node *node)
{
	...
	if (!(strncmp(mtkfb_lcm_name,"ilt9882n_ls_psc_hdp_dsi_vdo_lcm",strlen("ilt9882n_ls_psc_hdp_dsi_vdo_lcm")))) {
		...
		ILI_INFO("this is ili9882n_ls ilitek touchscreen\n");
	} else if (!(strncmp(mtkfb_lcm_name,"ilt9882n_txd_psc_ac_hdp_dsi_vdo_lcm",strlen("ilt9882n_txd_psc_ac_hdp_dsi_vdo_lcm")))) {
		ili_tp_module = MODEL_TXD;
		ILI_INFO("this is ili9882n_txd ilitek touchscreen\n");
	} else {
