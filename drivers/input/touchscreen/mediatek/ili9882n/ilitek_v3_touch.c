...
/*ritchie add for oppo*/
	...
	ilits->pre_y = touch_info[0].y;/*ritchie add for oppo*/
#ifdef ROI
	...
	if (ilits->finger) {
		...
		if (ilits->last_touch) {
			...
			input_sync(ilits->input);
			ilits->last_touch = 0;
			OPPO_DBG(1, "release [x,y] = %d, %d\n", ilits->pre_x, ilits->pre_y);/*ritchie add for oppo*/
		}
		ilits->first_report = true;/*ritchie add for oppo*/
	}
