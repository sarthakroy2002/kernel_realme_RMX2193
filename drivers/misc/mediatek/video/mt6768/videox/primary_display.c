...
#define FFL_END_LEVEL_2047 ((LED_2047) * 23 / 100)
#define FFL_END_LEVEL_4095 ((LED_4095) * 23 / 100)
...
int _set_cabc_mode_by_cmdq(unsigned int level)
{
	...
	if (primary_display_is_video_mode()) {
		mmprofile_log_ex(ddp_mmp_get_events()->primary_set_cmd, MMPROFILE_FLAG_PULSE, 1, 2);
		cmdqRecReset(cmdq_handle_lcm_cmd);
//Hao.Liang@ODM_WT.MM.Display.Lcd, 2020/11/24, Add cabc cmdq
			_cmdq_insert_wait_frame_done_token_mira(cmdq_handle_lcm_cmd);
...
//Jiantao.Liu@ODM_WT.MM.Display.Lcd, 2020/07/08, LCD backlight switch 11bit to 12bit
			if (1 == backlight_twelve_bit_flag)
				FFL_END_LEVEL = FFL_END_LEVEL_4095;
			else
				FFL_END_LEVEL = FFL_END_LEVEL_2047;
