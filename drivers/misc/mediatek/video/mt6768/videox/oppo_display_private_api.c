...
extern unsigned int backlight_high_light_dre_cabc;
...
#endif
//#ifdef ODM_WT_EDIT
//Tongxing.Liu@ODM_WT.MM.Display.Lcd, 2021/01/25, dre function in gamma
...
//Jiantao.Liu@ODM_WT.MM.Display.Lcd, 2020/07/08, LCD backlight switch 11bit to 12bit
	if (1 == backlight_twelve_bit_flag) {
		if (oppo_set_brightness > LED_4095 || oppo_set_brightness < LED_OFF) {
			return num;
		}
	}else{
...
{
//Jiantao.Liu@ODM_WT.MM.Display.Lcd, 2020/07/30, Modify  DRE&CABC for backlight hight light mode
	if (1 == backlight_high_light_dre_cabc) {
