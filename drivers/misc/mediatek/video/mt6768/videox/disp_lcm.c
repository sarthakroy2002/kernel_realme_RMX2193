...
extern unsigned int backlight_current_switch;
#endif
int _lcm_count(void)
{
	return lcm_count;
...
//Jiantao.Liu@ODM_WT.MM.Display.Lcd, 2020/07/27, switch of LCD backlight current 24.5MA
	...
	if ( level < 2048) {
		/* y= a*x^2 + b*/
		//value_a = ((BRIGHT_BL_Y - BRIGHT_BL_Y1) / (BRIGHT_BL_X * BRIGHT_BL_X - BRIGHT_BL_X1 * BRIGHT_BL_X1)) * VALUE_MASK;
		//value_b = (BRIGHT_BL_Y1 - value_a * (BRIGHT_BL_X1 * BRIGHT_BL_X1)) * VALUE_MASK;
...
//Jiantao.Liu@ODM_WT.MM.Display.Lcd, 2020/07/27, switch of LCD backlight current 24.5MA
		if (1 == backlight_current_switch)
			OPPO_BRIGHT_TO_BL_BIGGER(level_temp, level, BL_BIGGER_MIN, BL_MAX, BRIGHT_MIN,BRIGHT_MAX);
		else
			OPPO_BRIGHT_TO_BL(level_temp, level, BL_MIN, BL_MAX, BRIGHT_MIN,BRIGHT_MAX);
		pr_err("check brightness level_temp == %u \n",level_temp);
		return level_temp;
        }
	} else if (0 == level) {
	...
	void *handle, int level)
{
	...
	//Jiantao.Liu@ODM_WT.MM.Display.Lcd, 2020/07/08, LCD backlight switch 11bit to 12bit
		if (1 == backlight_twelve_bit_flag)
			level_temp = oppo_private_set_backlight(level);
		else
