...
#define PASCAL_DTNAME "mediatek,flashlights_monet"
#endif

#define PASCAL_NAME "flashlights_monet"
...
#define PASCAL_PINCTRL_PIN_TEMP_GPIO 3
...
#define PASCAL_PINCTRL_STATE_FLASH_TEMP_HIGH "flash_light_temp_pin_1"
#define PASCAL_PINCTRL_STATE_FLASH_TEMP_LOW  "flash_light_temp_pin_0"
...
static struct pinctrl_state *pascal_flash_temp_high;
static struct pinctrl_state *pascal_flash_temp_low;
...
static int pascal_pinctrl_init(struct platform_device *pdev)
{
    ...
    pascal_flash_temp_high = pinctrl_lookup_state(
        pascal_pinctrl, PASCAL_PINCTRL_STATE_FLASH_TEMP_HIGH);
    if (IS_ERR(pascal_flash_temp_high)) {
        printk("Failed to init (%s)\n", PASCAL_PINCTRL_STATE_FLASH_TEMP_HIGH);
        ret = PTR_ERR(pascal_flash_temp_high);
    }
    pascal_flash_temp_low = pinctrl_lookup_state(
        pascal_pinctrl, PASCAL_PINCTRL_STATE_FLASH_TEMP_LOW);
    if (IS_ERR(pascal_flash_temp_low)) {
        printk("Failed to init (%s)\n", PASCAL_PINCTRL_STATE_FLASH_TEMP_LOW);
        ret = PTR_ERR(pascal_flash_temp_low);
    }
	...
	switch (pin) {
		...
		case PASCAL_PINCTRL_PIN_PWM_GPIO:
			if (state == PASCAL_PINCTRL_PIN_STATE_LOW &&!IS_ERR(pascal_pwm_gpio_low))
				...
				printk("set err, pin(%d) state(%d)\n", pin, state);
			break;
		case PASCAL_PINCTRL_PIN_TEMP_GPIO:
			if (state == PASCAL_PINCTRL_PIN_STATE_LOW &&!IS_ERR(pascal_flash_temp_low))
				ret = pinctrl_select_state(pascal_pinctrl, pascal_flash_temp_low);
			else if (state == PASCAL_PINCTRL_PIN_STATE_HIGH &&!IS_ERR(pascal_flash_temp_high))
				ret = pinctrl_select_state(pascal_pinctrl, pascal_flash_temp_high);
			else
				printk("set err, pin(%d) state(%d)\n", pin, state);
			break;
		default:
...
//wenhui.chen@realme.DRV add for flashlight NTC
int pascalI_temperature_enable(bool On)
{
    if(On) {
        pascal_pinctrl_set(PASCAL_PINCTRL_PIN_TEMP_GPIO, PASCAL_PINCTRL_PIN_STATE_HIGH);
    } else {
        pascal_pinctrl_set(PASCAL_PINCTRL_PIN_TEMP_GPIO, PASCAL_PINCTRL_PIN_STATE_LOW);
    }
